...
async function initPostgres(){
  ...
  // Idempotent seed: restore any missing demo content even if the database is only partially populated.
  for(const c of seed.courses) await sql('INSERT INTO courses(id,exam,title,description,skills,published) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(id) DO UPDATE SET exam=EXCLUDED.exam,title=EXCLUDED.title,description=EXCLUDED.description,skills=EXCLUDED.skills,published=EXCLUDED.published',[c.id,c.exam,c.title,c.description,JSON.stringify(c.skills),c.published]);
  for(const l of seed.lessons) await sql('INSERT INTO lessons(id,course_id,exam,skill,title,body,objectives,published) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(id) DO UPDATE SET course_id=EXCLUDED.course_id,exam=EXCLUDED.exam,skill=EXCLUDED.skill,title=EXCLUDED.title,body=EXCLUDED.body,objectives=EXCLUDED.objectives,published=EXCLUDED.published',[l.id,l.courseId,l.exam,l.skill,l.title,l.body,JSON.stringify(l.objectives),l.published]);
  for(const p of seed.plans) await sql('INSERT INTO plans(id,name,price,days,featured,course_ids,features,published) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,price=EXCLUDED.price,days=EXCLUDED.days,featured=EXCLUDED.featured,course_ids=EXCLUDED.course_ids,features=EXCLUDED.features,published=EXCLUDED.published',[p.id,p.name,p.price,p.days,p.featured,JSON.stringify(p.courseIds),JSON.stringify(p.features),p.published]);
  for(const t of seed.teachers) await sql('INSERT INTO teachers(id,name,bio,photo_url) VALUES($1,$2,$3,$4) ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,bio=EXCLUDED.bio,photo_url=EXCLUDED.photo_url',[t.id,t.name,t.bio,t.photoUrl]);
  for(const c of seed.classes) await sql('INSERT INTO classes(id,title,exam,date,time,instructor,teacher_id,link,description) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT(id) DO UPDATE SET title=EXCLUDED.title,exam=EXCLUDED.exam,date=EXCLUDED.date,time=EXCLUDED.time,instructor=EXCLUDED.instructor,teacher_id=EXCLUDED.teacher_id,link=EXCLUDED.link,description=EXCLUDED.description',[c.id,c.title,c.exam,c.date,c.time,c.instructor,c.teacherId,c.link,c.description]);
  for(const q of seed.questions) await sql('INSERT INTO questions(id,test,prompt,options,answer) VALUES($1,$2,$3,$4,$5) ON CONFLICT(id) DO UPDATE SET test=EXCLUDED.test,prompt=EXCLUDED.prompt,options=EXCLUDED.options,answer=EXCLUDED.answer',[q.id,q.test,q.prompt,JSON.stringify(q.options),q.answer]);
...
app.get('/api/courses',async(q,r)=>{try{const d=await getAll();r.json((d.courses||[]).filter(x=>x.published!==false).map(c=>({...c,lessons:(d.lessons||[]).filter(l=>l.courseId===c.id&&l.published!==false).length})));}catch(e){console.error(e);r.status(500).json({error:'Could not load courses.'});}});
app.get('/api/plans',async(q,r)=>{try{const d=await getAll();r.json((d.plans||d.pricing||[]).filter(x=>x.published!==false));}catch(e){console.error(e);r.status(500).json({error:'Could not load plans.'});}});
